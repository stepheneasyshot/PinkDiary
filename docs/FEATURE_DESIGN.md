...
| 排卵日（Ovulation） | 通常位于下次经期前约 14 天 | **初期不做**，预留 |
...
| 日历 | 状态标记 | 经期日、预测经期日、今天、选中日期、非本月日期 |
...
|------|------|----------|
| 普通日期 | 无背景 | — |
| 经期日（已记录） | 实心圆 | 主题粉 `#F48FB1` / 主色 |
| 预测经期日 | 空心圆/虚线圆 | 浅粉描边 |
| 今天 | 高亮描边 | 主题深色边框 |
| 选中日期 | 填充/描边 | 主题强调色 |
| 非本月日期 | 淡灰 | `onSurface` 40% 透明度 |

- 已结束经期：起止日之间连续填充（柔和粉色带）。
- 进行中经期：从开始日到「今天」填充，结束待定。
- 预测区间：起止日之间的空心圆 + 浅色底。
...
2. 按钮随上下文变化：
   ...
   - 该日是进行中经期的开始日 → 「标记经期结束」
   - 已存在完整记录 → 「删除这条记录」
3. 快捷入口：首页提供「今天来月经了」一键标记（自动处理开始/进行中逻辑）。
...
| M2 日历 UI | 月视图、切换月份、状态标记、图例 | 可正确渲染经期/预测/今天/选中 |
